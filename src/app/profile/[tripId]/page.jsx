...
const SingleTrip = ({ params }) => {
  const { username, trips } = mockUserProfile;
  const trip = trips.find(({ id }) => id === params.tripId);
  const tripImages = trip.tripImages;
  const { name, description, displayDate, tripActivities, location, tripBudget, tips, likes } = trip;

  const [previousImage, setPreviousImage] = useState(trip.tripImages[0].tripImage);
  const [opacity, setOpacity] = useState(1);
  const [isImageSelected, setIsImageSelected] = useState(false);
  const [currentImage, setCurrentImage] = useState(trip.tripImages[0].tripImage);
  ...
  } else if (tripActivities.length > 3 && tripActivities.length <= 7) {
              ...
              <div className="grid grid-cols-5 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-5 2xl:grid-cols-5 gap-2 h-24">
                {tripImages.slice(1).map((tripImage, id) => (
                  <div
                    ...
                    onMouseEnter={() => {
                      setCurrentImage(tripImage.tripImage);
            ...
            <div className="flex flex-col w-11/12 mx-auto md:w-full md:mx-0 lg:w-1/2">
              <div className="flex justify-between border-b border-black/50 lg:pb-3 pb-5">
                <div className="flex gap-3 items-center">
                  <DestinationIcon />
                  <h3 className="secondary-header text-black">{location}</h3>
                </div>
              ...
              <div className="border-b border-black/50 lg:py-3 py-5 flex flex-col gap-2">
                ...
                <ul className={`grid grid-cols-2 lg:${gridClass} gap-2`}>
                  {tripActivities.map((activity, description, id) => (
                    <div className="flex items-center gap-1 h-8" key={`${activity.name}${id}`}>
                      <div className="">{activityIcons[activity.name]}</div>
                      <li className="">{activity.name}</li>
                    </div>
                  ))}
                </ul>
              </div>
